...
-- Argus Flow — DB 기반 인증·인가 스키마 v1 (PostgreSQL)
--
-- 적용:  bin/argus-user.sh schema-init
--        또는 psql -f V1__baseline.sql
--
-- id 와 identity 를 분리한 이유: NiFi 의 접근 정책은 User.getIdentifier() 를 참조한다.
-- identity(로그인 ID)를 PK 로 쓰면 로그인 ID 변경이 곧 권한 상실이 된다.
...
INSERT INTO argus_schema_version (version)
VALUES (1)
